...
const input = "input";
const size: u8 = 141;
var start = Coordinate{ .x = size - 2, .y = size - 2, .d = Direction.west };
...
fn reconstructPath(cameFrom: Map(Coordinate, Coordinate), current: Coordinate) void { // []Coordinate {
    ...
    while (cameFrom.get(cursor)) |cursor2| {
        cursor = cursor2;
        array[cursor.y][cursor.x] = '*';
        bestPlaces.put(Coordinate{ .x = cursor.x, .y = cursor.y, .d = Direction.north }, false) catch unreachable;
...
var bestPlaces: Map(Coordinate, bool) = undefined;
...
fn order(_: void, r: Coordinate, l: Coordinate) std.math.Order {
    ...
    return std.math.order(fScore.get(r).? * 100000000 + r_x * 1000 + r_y, fScore.get(l).? * 100000000 + l_x * 1000 + l_y);
...
fn aStar() !?u64 {
    ...
    bestPlaces = Map(Coordinate, bool).init(allocator);
    defer bestPlaces.deinit();
    ...
    while (openSet.removeOrNull()) |current| {
        ...
        if (current.y == 1 and current.x == 1) {
            print("a {d}\n\n", .{gScoreCurrent});
        ...
        for (getWaysOut(current)) |neighbour| {
            ...
            const tentativeGScore = gScoreCurrent + d(current, neighbour);
    ...
    // if (last) |last2| {
    //     reconstructPath(cameFrom, last2);
    // }

    return bestPlaces.count(); // Failure: open set is empty but goal was never reached
}
